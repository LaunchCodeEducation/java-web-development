...
In the :ref:`previous chapter <sql-part1-exercises>`, we set up a database to store information about seeds. Let's expand that idea now that we know all about foreign keys!
...
#. ``zone`` is an integer for the hardiness zone where our plant will live in best.
#. ``season`` is a string that specifies the season that the plant blooms in.
...
#. ``date_planted`` of type ``DATE`` for the day that we planted the plant on.
...
Use an inner join on ``seeds`` and ``garden_bed`` to see which plants are in our garden bed that we also have seeds for.
