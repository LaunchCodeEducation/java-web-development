...
potentially larger than they need to be. Like ArrayLists, we can index into
Arrays with integers (``students[i]`` for example). Unlike ArrayLists,
however, there is no analog of ``.add()``, which adds an item to the
end of a ArrayList. We must always access and assign Array elements using an
